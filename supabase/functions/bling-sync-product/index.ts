...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (error instanceof BlingHttpError) {
      ...
      return fail(request, productSyncBlingErrorMessage(error), error.temporary ? 503 : error.status, {
    ...
    if (message === "bling_sync_lock_lost") return fail(request, productSyncValidationMessage(message), 409, { code: message });

    const knownMessage = productSyncValidationMessage(message);
    if (knownMessage !== productSyncValidationMessage("unknown")) {
